...
export default function TextImageSection({
  ...
  const defaultTitleColor = isDarkBackground
    ? "var(--bs-secondary)"
    : isBlueBackground
      ? "var(--bs-dark)"
      : "var(--bs-primary)";
  const defaultSubtitleColor = isDarkBackground
    ? "var(--bs-primary)"
    : isBlueBackground
      ? "var(--bs-dark)"
      : "var(--bs-primary)";
  ...
  return (
    ...
    >
      <div className="container py-5">
        <div
          className={`row align-items-center flex-column flex-md-row${imagePosition === "right" ? " flex-md-row-reverse" : ""}`}
        >
          {/* Imagen circular */}
          ...
          <div className="col-12 col-md-6 text-center text-md-start order-1 order-md-2">
            ...
            {subtitle && (
              <h3 className="fw-bold mb-2">
                <span style={{ color: subtitleColor || defaultSubtitleColor }}>
                  {subtitle.split(" ")[0]}
                </span>{" "}
            ...
            {paragraphs.map((text, idx) => (
              <p key={idx} className="mb-2" style={{ color: textColor }}>
                {text}
              </p>
            ))}
          </div>
